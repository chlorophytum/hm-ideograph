...
import { getRecPath, MultiStrokeProps } from "./props";

export namespace MultiStrokeHint {
	const TAG = `${PREFIX}::MultiStrokeHint`;
	export class Hint implements IHint {
		private readonly N: number;
		constructor(private readonly props: MultiStrokeProps) {
	...
	export class HlttCompiler implements IHintCompiler {
		constructor(
			...
			private readonly props: MultiStrokeProps
		) {}
		public doCompile() {
			const { props } = this;
			const N = props.middleStrokes.length;
